...
---------------------------------------------------------------------------------
-- Simply-Typed Lambda Calculus with Disjoint Sums and Type-directed Reduction
---------------------------------------------------------------------------------
...
-- Closure under Anti-InL & Anti-InR
...
-- Closure of sn under βι-Expansion below Evaluation Contexts
---------------------------------------------------------------------------------
...
-- Closure of SN under Evaluation Context Composition
∘C^SN : ∀ {Γ α β σ c c′} → Γ ∣ β ⊢SN σ ∋ c → Γ ∣ α ⊢SN β ∋ c′ → Γ ∣ α ⊢SN σ ∋ c ∘C c′
∘C^SN <>                   c′^SN = c′^SN
∘C^SN (app c^SN t^SN)      c′^SN = app (∘C^SN c^SN c′^SN) t^SN
∘C^SN (cas c^SN l^SN r^SN) c′^SN = cas (∘C^SN c^SN c′^SN) l^SN r^SN
...
-- Closure of sn under Head βι-Expansion
...
-- Elimination (i.e. One-Layer Deep Evaluation Contexts)
...
-- Redexes: Either β or ι rule can fire
...
mutual
  ...
  -- Open up a term which starts with an elimination to either discover
  -- it's neutral or find the head redex
...
-- Logical Predicate Transformer for Sums
infix 3 _+𝓡_
...
-- Reifying Dead Branches with Dummy Variables (used in a stuck Case)
...
-- Morally a form of Application (used when a Case's Branch fires)
[/0]^𝓡 :
